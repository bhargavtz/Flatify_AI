...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoadingAuth && !currentUser) {
      router.push('/login'); 
    } else if (!isLoadingAuth && currentUser && !isLoadingRole && !userRole) {
      router.push('/'); 
    }
  }, [currentUser, isLoadingAuth, userRole, isLoadingRole, router]);

  if (isLoadingAuth || !currentUser || (currentUser && isLoadingRole)) {
    return (
  ...
  if (currentUser && !userRole && !isLoadingRole) {
     return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <LoadingSpinner size="xl" /> 
      </div>
        ...
        <div className="container flex items-center justify-between h-16 max-w-screen-2xl">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <span className="text-sm font-medium text-muted-foreground hidden sm:inline">
              User: <span className="text-primary font-semibold">{currentUser?.name || 'Guest'}</span> | Mode: <span className="text-primary font-semibold capitalize">{userRole}</span>
            </span>
            <DropdownMenu>
